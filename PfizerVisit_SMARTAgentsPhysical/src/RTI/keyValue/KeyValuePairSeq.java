...
/**
 * A sequence of KeyValuePair instances.
 */
public final class KeyValuePairSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
        // Package Fields
        // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
        ...
        public KeyValuePairSeq() {
        super(KeyValuePair.class);
    }

    public KeyValuePairSeq (int initialMaximum) {
        super(KeyValuePair.class, initialMaximum);
    }

    public KeyValuePairSeq (Collection elements) {
        super(KeyValuePair.class, elements);
    }
        ...
        // copy the data from source into this (into positions that already
            ...
            if (typedSrc.get(i) == null){
                ...
                // check to see if our entry is null, if it is, a new instance has to be allocated
                    ...
                    set(i, KeyValuePair.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyValuePair objects (beyond the original size of this object)
